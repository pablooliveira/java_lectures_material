...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Java programs run inside a Virtual Machine (JVM), "a software implementation" of a computer.
          (this makes Java programs portable among different architectures).
    \item For the sake of performance and space, the JVM reads bytecode (a low level language).
    \item It is easier for humags to write programs in a more expressive, high level language, like Java.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item
  Thus we introduce \structure{types},
  that tell us how a value should be used.
  \item
  ...
  \begin{example}
   ...
   \pause
   \lstinline!long molecules;! &  64 bit defined on $[-2^{64}..2^{64}-1]$ \\
   \hline
   \pause
...
\begin{frame}[fragile]
  \frametitle{Mutable variables, assignement, final variables}
...
// initialise the values of the elements
price[0] = 1.00;
price[1] = 5.99;
price[2] = 3.25;
...
\begin{frame}[fragile]
  ...
  \begin{alertblock}{Caution}
    \begin{itemize}
    \item Strings and arrays are a mix between objects and primitive types. 
    \item Some care must be taken when manipulating strings and arrays. 
    Particularly, strings and arrays are references, and strings are immutable.  

    \item Do not worry: we will talk about this in the next lecture.
    \end{itemize}
  \end{alertblock}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{example}
   \begin{longtable}{l|r}
   \lstinline!a = 5; b[4] = 30!& assignement\\
   \pause
   ...
   \pause
   \lstinline!4 * 4! $\rightarrow$ \lstinline!20! & multiplication \\
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
   \item Increment (\lstinline!++!), decrement (\lstinline!--!)  operators:
  \begin{code}
    int a,b;
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item Operation + Assignement (\lstinline!+= -= *= /=! etc ...)
  \begin{code}
     a += b;    // equivalent to:
     a = a + b; 
  \end{code}
  ...
  \begin{code} 
    ...
    a != c; // --> true
  \end{code}
  \item \alert{Referential equality when used with objects and strings (we'll come back to this later)}
  \end{itemize}
\end{frame}
...
\begin{example}
  ...
  has some methods: \alert{turn, advance and readPosition}.
  \begin{center}
  \only<2>{
...
\begin{frame}[fragile]
  ...
  \item \lstinline!car1.readPosition()! returns the position of \structure{the specific instance turtle1}:
...
\begin{frame}[fragile,shrink]
  \frametitle{Static Methods/Variables}
    \structure{Q: sharing a common variable between all the objects in a class ?}
    \begin{code}
class Turtle {
  static int step_size = 5;
    ...
    \end{code}
    \pause
    \structure{Q: writing a method independant from a particular instance ?}
    \begin{code}
class Turtle {
